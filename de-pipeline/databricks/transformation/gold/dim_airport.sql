...
-- FLAG: re-keyed on icao alone (silver's key is icao+iata+name); dedup picks one row per
-- icao+_loaded_at (tie-break by iata) - revisit if that tie-break is wrong.
...
USING (
  WITH incoming AS (
    ...
    QUALIFY ROW_NUMBER() OVER (PARTITION BY icao, _loaded_at ORDER BY iata) = 1
  ),
  combined AS (
    SELECT icao, effective_start AS _loaded_at FROM {{CATALOG}}.gold_flights.dim_airport WHERE is_current
    UNION ALL
...
USING (
  ...
  SELECT
    md5(concat_ws('||', icao, CAST(_loaded_at AS STRING))) AS airport_sk,
    icao, iata, name, country, country_code, continent, iso_region, municipality,
